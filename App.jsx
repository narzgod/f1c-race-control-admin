          ...
          {!target && (
            <div>
              ...
              <input
                ...
                placeholder="MISAL: BE READY / GO! GO! GO!"
                className="f1-input f1-input-w w-full mt-2 px-4 py-3 font-bold uppercase text-sm"
              />
              <p className="font-semibold mt-2" style={{ fontSize: 10, color: MUTED_DARK, lineHeight: 1.5 }}>
                Notifikasi di layar driver akan berkedip bergantian tiap 0.7 detik: teks utama (dengan
                latar warna) lalu teks ini (tanpa latar). Kosongkan kalau tidak perlu teks kedua.
              </p>
            </div>
          )}

          {target && (
            <p className="font-semibold" style={{ fontSize: 10, color: MUTED_DARK, lineHeight: 1.5 }}>
              Karena item ini butuh driver target, layar driver otomatis berkedip antara{" "}
              <span className="text-white">nama driver</span> (dengan latar warna) dan{" "}
              <span className="text-white">
                {allowTarget ? "nama item" : "nama item"}
                {" "}
                {`"${name || "..."}"`}
              </span>{" "}
              (tanpa latar) — tidak perlu Secondary Text.
            </p>
          )}

          <div>
            <Label>Custom Text Color (opsional)</Label>
            <div className="flex items-center gap-2 mt-2">
              <input
                value={textColor}
                onChange={(e) => setTextColor(e.target.value)}
                placeholder="#FFFFFF atau white"
                className="f1-input f1-input-w flex-1 px-4 py-3 font-bold text-sm"
              />
              <span
                className="flex-shrink-0 rounded-full"
                style={{ width: 28, height: 28, background: textColor.trim() || "transparent", border: "1px solid #555" }}
              />
            </div>
            <p className="font-semibold mt-2" style={{ fontSize: 10, color: MUTED_DARK, lineHeight: 1.5 }}>
              Kosongkan untuk warna otomatis. Isi kalau warna teks default kurang jelas di background
              yang terlalu terang/gelap (contoh: #000000 untuk teks hitam).
            </p>
          </div>
